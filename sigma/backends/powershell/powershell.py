...
class PowerShellBackend(TextQueryBackend):
    ...
    def finalize_query_custom(self, rule: SigmaRule, query: Any, index: int, state: ConversionState) -> Any:
        ...
        if hasattr(rule, "eventid"):
            ...
            return f'Import-Clixml -Path <SERVICE_PATH>'+ f"|Where-Object {{($_.EventID -ne $null) -and ({query})}}" + rule_id
        else:
            exists_query = '('+' -and '.join(['($_.%s -ne $null)' % s for s in set(fields.findall(query))])+')'
            return f'Import-Clixml -Path <SERVICE_PATH>'+ f"|Where-Object {{{exists_query} -and ({query})}}" + rule_id
